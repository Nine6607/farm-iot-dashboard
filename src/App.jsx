...
// เปลี่ยนจาก PRODUCTS เป็น SERVICES และใส่รายละเอียดงานของท่าน
const SERVICES = [
...
function App() {
  ...
  return (
    // เปลี่ยนโทนสี selection เป็นสีฟ้า (cyan)
    <div class="min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-cyan-500 selection:text-white pb-20">
      ...
      <nav class="flex justify-between items-center p-6 border-b border-slate-800 bg-slate-900/80 backdrop-blur-xl sticky top-0 z-50">
        <div class="text-2xl font-black text-cyan-400 flex items-center gap-2">⚙️ PNPK TEAM</div>
        <button onClick={() => setIsCartOpen(true)} class="relative bg-white text-black px-6 py-2 rounded-full font-bold hover:bg-cyan-400 transition-all active:scale-95 shadow-xl">
          Quotation {cartItems().length > 0 && `(${cartItems().length})`}
        </button>
      </nav>

      <main class="max-w-7xl mx-auto p-6">
        <header class="py-16 text-center">
          {/* เปลี่ยนคำโปรยหลักให้ดูเป็นงานวิศวกรรม */}
